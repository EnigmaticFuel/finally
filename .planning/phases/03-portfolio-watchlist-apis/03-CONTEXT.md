...
- **D-13:** **Reset has no API-level guard** — no confirmation token, no required body field. `POST` and it happens, exactly like a trade. It is fake money in a single-operator localhost simulator, and PLAN.md deliberately gives trades no confirmation dialog; any confirmation belongs in the Phase 4 UI where the user actually clicks. Recorded so Phase 4 owns that step and it is not dropped.
  ...
  **D-13 Amendment — 2026-08-14 (WR-02).** The original decision above stands as written and is not rewritten; this paragraph records what changed and why, in the same form `03-SEAM-CONTRACT.md` carries its amendment. Plan 03-09 gives `POST /api/portfolio/reset` a **required JSON body**, `ResetRequest` with a single `confirm: bool` field. This is **transport hardening, not confirmation UX**, and the distinction is the whole of the amendment.

  What D-13 refused was a *guard*: a token to look up, a field whose value gates the action, a server-side "are you sure" the user has to satisfy. None of that is added. **The `confirm` value is never read** — no handler branches on it, and plan 03-09 pins that mechanically (`grep -c "body\.confirm" backend/app/api/portfolio.py` outputs `0`). The security property comes entirely from *requiring a JSON body at all*: that forces `Content-Type: application/json`, which makes a cross-origin POST a non-simple request needing a preflight the app answers with no CORS headers. A cross-origin HTML form, a `navigator.sendBeacon` and a `no-cors` `fetch` are each refused with 422 before the reset transaction opens. WR-02 in `03-REVIEW.md` demonstrated that vector with working attacker HTML against an unauthenticated, destructive endpoint; "fake money on localhost" bounds the damage but does not close the vector, because the request lands from any tab the user has open while the container runs.

  **Confirmation UX remains Phase 4's concern, exactly as D-13 assigned it.** Phase 4's UI is still where the user is asked and still where the click happens; the API adds no second asking. The one consumer-visible consequence, flagged for the developer: Phase 4's UI must send `{"confirm": true}` as JSON — a client that posts the endpoint bare stops working. This is the only API contract change in the four gap-closure plans, and PORT-14 is `[NEW]` with no `planning/PLAN.md` text, so it contradicts no published shape.
