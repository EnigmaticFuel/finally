...
- **Ready for plan `02-02` (start/stop scripts).** The exact `docker run` line the scripts must wrap is proven: `-p 127.0.0.1:8000:8000`, `-v <repo-root>/db:/app/db`, `--env-file .env`, image `finally-app:latest`, container name `finally-app`. Note for `02-02`: the container was left **running** at the end of this plan, so an idempotency check (D-09) has a live subject to detect.
