...
No external API integration: this phase only containerizes Phase 1's existing FastAPI app and adds host lifecycle scripts. No external SDK, service client, or third-party HTTP call is added.

The only "API" terms in scope are this project's own first-party routes (`/api/health`, `/api/stream/prices`), which Phase 1 already built and this phase only exercises over HTTP.
